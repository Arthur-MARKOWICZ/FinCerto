            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {contas.map((conta) => (
                <div
                  ...
                  className={`bg-white rounded-xl shadow-sm p-6 cursor-pointer hover:shadow-md transition-all duration-200 border-2 ${
                    contaAtualId === conta.id ? 'border-blue-500 shadow-blue-100' : 'border-transparent hover:border-blue-200'
                  }`}
                >
                  ...
                  <div className="space-y-2">
                    ...
                    {conta.id && (
                      <div className="flex justify-between items-center px-1">
                        <span className="text-gray-500 text-sm">ID da Conta</span>
                        <span className="text-gray-400 text-sm font-mono">#{conta.id}</span>
                      </div>
                    )}
                  </div>

                  <div className="mt-4 pt-4 border-t border-gray-100">
                    <div className="text-sm text-blue-600 font-medium text-center hover:text-blue-700">
                      {contaAtualId === conta.id ? 'Conta atual - Ver detalhes →' : 'Clique para ver detalhes →'}
                    </div>
                  </div>
                </div>
              ))}
